...
\section{Gateux differentiability}
...
Note that to be precise, $\bar{u}$ is an extension of the Dirichlet datum $f$, on the moving domain $T_k(T(U_r))$. Because of this, let's fix $\bar{u}_T$ with this property on $T(U_r)$. We show that $\bar{u}_T\circ T_k^{-1}$ satisfies the conditions stated in \cref{pb:diri_ext}. 

In particular:

\begin{itemize}
	\item composition with $T$ preserves the smoothness of the extension, as seen in \cref{prop:bochner_bound}, given that $\circ T_k^{-1}$ is a linear bounded operator between $V_T$ and $V_{T_k \circ T}$
...
\begin{align*}
L_T(k,w,v_0,q,p) = \\
\frac{1}{2}\int_I \int_{T(U_r)}|v_0+\bar{u} - w|^2|\det(DT_k)|+\\
\int_I ( w_t , q |\det(DT_k)|)_{H_T}+ (A_{T_k}\nabla w, \nabla q)_{H_T} -\int_I(g,\tr_{U} q)_{L^2(\Gamma_f)} +\\ \int_I (v_{0t},p |\det(DT_k)|)_{H_T} + (A_{T_k} \nabla v_0, \nabla p)_{H_T}+\\\int_I(\bar{u}',p|\det(DT_k)|)_{H_T}+(A_{T_k} \nabla \bar{u} , \nabla p)_{H_T}
\end{align*}
...
We also call $b(k, u) = \frac{1}{2}\int_I \int_{T(U_r)}|v_0+\bar{u}_T - w|^2|\det(DT_k)|$ and $a(k, u,\pi) = G(k,u,\pi)-b(k, u)$, $E = W_0(I, V_T)\times W_0(I,W_T)$, $F=W^0(I, V_T)\times W^0(I, W_T)$.
...
In particular $\pi^k = (Q^k \circ \tau^k,P^k \circ \tau^k)$, where we introduced the average adjoint problems:
...
p^\tau(T)=0\\
p^\tau = 0 \text{ on } \Sigma_m \sqcup \Sigma_f
\end{align*}
...
Now, the application $\delta \te \mapsto \id +\delta \te \circ \tau^{-1}$ is Frechet differentiable at $\delta \te =0$, as a map of $\Te$ into $\cV^1$, with Frechet derivative $\delta \circ \te \tau^{-1}$, which is linear and bounded by point i) of lemme 2.2, \cite{murat}. Note, we needed here $\tau \in \cT^1$.
...
\tred{NB: here I need $f_2(0) \in V$ probably, so that we are using the compatibility condition!}.
