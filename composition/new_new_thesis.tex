...
% Colors
\tcolorboxenvironment{thm}{colback=black!5!white,colframe=black!5!black, opacityframe=0}
\tcolorboxenvironment{cor}{colback=black!5!white,colframe=black!5!black, opacityframe=0}
\tcolorboxenvironment{lemma}{colback=black!5!white,colframe=black!5!black, opacityframe=0}
\tcolorboxenvironment{defn}{colback=black!5!white,colframe=black!5!black, opacityframe=0}
\tcolorboxenvironment{prop}{colback=black!5!white,colframe=black!5!black, opacityframe=0}
\tcolorboxenvironment{ass}{colback=black!15!white,colframe=black!5!black, opacityframe=0}
\tcolorboxenvironment{pb}{colback=black!10!white,colframe=black!5!black, opacityframe=0}
...
The overall problem, comprehending \cref{pb:diri}, \cref{pb:neu}, is:
...
Note, for the regular case, we will be discussing only a homogeneous initial condition. This will suffice for our purposes. A generalization will entail handling more complicated compatibility conditions, see \cite{lions}, Chapter 2. Also note that the requirements imposed on the data below, are only sufficient, to ensure the desired regularity.
...
We define a polygonal/polyhedral meshing $\Omega_h$ of $\Omega$, which has boundary nodes on $\partial \Omega$. We denote by $\Gamma_{D,h}, \Gamma_{N,h}$ the discrete Dirichlet and Neumann boundaries, and by $S^1_{h,D} $ the space of piecewise linear lagrangian FEM $S^1_h$ which are zero on $\Gamma_{D,h}$.

We collect some useful tools to relate $\Omega$ and $\Omega_h$.

\begin{prop}[Deformation into smooth boundary]
Assume a quasi-uniform mesh. There exists, for $h$ small enough, $G_h: \Omega_h \rightarrow \overline{\Omega}$ satisfying:

\begin{itemize}
	\item $G_h$ is a homeomorphism \textcolor{red}{Injectivity is actually not so easy, let's assume this}
...
\begin{itemize}
	\item if $v\in H^m(T)$ if and only if $v^{-l} \in H^m(G_h(T))$, for $m=0,1$
...
The second and third points follow by applying the arguments about conformity outlined in section 5 of \cite{bernardi}. Following again Example 2 therein, we discover that we can apply Proposition and Corollary 5.1.
...
\label{prop:lin_appr}
\textcolor{red}{Prop. 4.6 of Edelmann}, add this.
\end{prop}
...
\begin{ass}[Smoothness on solution]
...
\begin{itemize}
	...
	\item $g_N \in H^1(I, H^{2}(\Gamma_D))$, so that, with reference to \cref{prop:trace}, we have $G_D:=Eg_D \in H^1(I,H^2(\Omega))$ and therefore (see \cref{lemma:bochner_Hk_map}), there holds	 $G_{D,h}:=\Pi_h G_D \in H^1(I, S^1_h)$ and $g_{D,h}:=G_{D,h}|_{\Gamma_{D,h}} \in H^1(I, S^1_h(\Gamma_{D,h}))$ (note, $g_{D,h} = \Pi_h g_D$)
	\item $f \in L^2(I,L^2(\Omega))$ and $f_h \in L^2(I, S^1_h)$, with error bound  $\norm{f-f_h^l}\lesssim C_f h^2$, for a.e. $t$, and also $\norm{f_h}_{H^1(\Omega_h)}\lesssim C_f$, $C_f$ independent of $h$ and belonging to $L^2(I)$.
...
A function $\delta_h \in H^1(I, S^1_{D,h})$ can be written as $u_h=\sum_j d_j(t)v_{hj}$, for the usual finite element basis $\{v_{hj}\}_j$ of $S^1_{D,h}$. We employ the splitting technique $\delta_h = u_h - G_{D,h}$, where $G_{D,h} \in H^1(I,S^1_h)$ is a suitable extension. By testing with the equation of \cref{pb:inh_parabolic_discr} with the basis functions $v_{hj}$ we obtain the problem:
\begin{align*}
	Md'(t)+Ad(t)= F(t), \text{ a.e. } t\\
	d(0) = D_0	
\end{align*}

Here, $M_{ij} = (v_{hi}, v_{hj})_{L^2(\Omega_h)}, A_{ij} = a(v_{hi}, v_{hj})$ are the so called mass and stiffness matrices, both invertible. We also have $F_j(t):= - (\partial_t G_{D,h}, v_{hj})_{L^2(\Omega_h)} - a_h(G_{D,h}, v_{hj}) + (f_h, v_{hj})_{L^2(\Omega_h)} + (g_{N,h}, v_{hj})_{L^2(\Gamma_{N,h})}$, together with $D_0:= u_{0h} - G_{D,h}$ (we are able to come to this problem thanks to the assumed compatibility between $u_{0h}, g_{D,h}$).
...
\begin{align*}
	...
	\norm{f-f_h^l}_{L^2(\Omega)}\norm{v_h}_{H^1(\Omega_h)} + h^2 C_f\norm{v_h}_{H^1(\Omega_h)}\lesssim C_f h^2 \norm{v_h}_{H^1(\Omega_h)}
\end{align*}
...
\begin{align*}
	\frac{1}{2} \norm{\theta_h(t)}_{L^2(\Omega_h)}^2 + \frac{1}{2}\int_0^t\norm{\theta_h}^2_{H^1(\Omega_h)} \lesssim 2\left (\int_0^t\frac{1}{2}\norm{\theta_h}^2_{L^2(\Omega_h)} + \frac{t}{2}\int_0^t\norm{\theta_h}^2_{H^1(\Omega_h)}\right) + 2h^4 \int_0^t Q^2 \\+ \frac{1}{2} \norm{\theta_h(0)}_{L^2(\Omega_h)}^2
\end{align*}
...
+ Ch^2\int_0^t C_f^2 + \frac{1}{6}\int_0^t\norm{\partial_t \theta_h}^2_{L^2(\Omega_h)}\\ 
...
Under \cref{ass:smoothness_par_discr}, \cref{ass:discr_reg} and \cref{ass:basic_par_mix} we have, for all $w \in L^2(I,H^1(\Omega))$:
...
For $w \in H^1(\Omega)$ we estimate $(\partial_t(\theta_h^l), w)_{L^2(\Omega)} = (w, (\partial_t\theta_h^l))_{L^2(\Omega)} =  (\pi_h w,\partial_t\theta_h)_{L^2(\Omega_h)}$. We have also used that lifting and differentiating with respect to time commute, by \cref{prop:lift} and \cref{lemma:bochner_Hk_map}. We can now apply \cref{eqn:theta}:
...
\begin{align*}
	\left | \int_I(\partial_t(\theta_h^l), w)_{L^2(\Omega)} \right | \lesssim\\
	\left ( \sqrt{\int_I \norm{\theta_h}_{H^1(\Omega_h)}^2}+ h^2 \sqrt{\int_I (C_f^2 + \norm{g_N}^2_{H^2(\Gamma_N)} + \norm{\partial_t u}^2_{H^2(\Omega)} + \norm{\partial_t g_D}^2_{H^2(\Gamma_D)} )}\right) \sqrt{\int_I\norm{ w}_{H^1(\Omega)}^2} \lesssim \ind{\cref{eqn:theta_energy}} \lesssim\\
	h^2 \left (  \sqrt{\int_T Q^2} + \norm{u_0}_{H^2(\Omega)} + \sqrt{\int_I (C_f^2 + \norm{g_N}^2_{H^2(\Gamma_N)} + \norm{\partial_t u}^2_{H^2(\Omega)} + \norm{\partial_t g_D}^2_{H^2(\Gamma_D)} )}\right) \norm{w}_{L^2(I,H^1(\Omega))}
\end{align*}

We can also estimate:
\begin{align*}
\left | \int_I (\rho_t, w)_{L^2(\Omega)}\right |\lesssim \ind{as in the proof of \cref{thm:semidiscrete_error_bound}}\lesssim\\
...
\textcolor{red}{To be precise we don't know whether $G_h$ takes measurable functions into measurable functions. But for $L^2$ arguments, we can reason as follows: $w^{-l}$ we know to be piecewise in $L^2$, so we can glue it to an $L^2$ fcn by in case modifying it on a null set. Also $\xi$ is piecewise measurable (it is peicewise $C^1$). So $w^{-l}\xi$ is measurable and $L^2$}
...
Applying \cref{prop:lift} we see that $w_h$ satisfies all the requirements.
