...
\begin{itemize}
	...
	\item $G_h|_T$ is of class $C^2(T)$ for all $T$ simplex composing $\Omega_h$
	\item $G_h|_T = \id$ on interior simplices $T$
...
Note, we assumed $g_D \in H^{3/2}(\partial \Omega)$. Applying Sobolev embeddings will yield the continuity of $g_D$ only for $n=2$. But thanks to the $C^2$ smoothness of $\partial \Omega$ we can apply theorem 1.5.1.2 of \cite{grisvard} and obtain a $G \in H^2(\Omega)$ with $\tr_2 G = g_D$. Here $\tr_2$ denotes the trace operator from $H^2(\Omega)$. By Sobolev embeddings, $G \in C(\overline{\Omega})$. 
...
Unfortunately the smoothness assumptions to obtain optimal order of convergence are quite strict. However, the advantage of this approach is that it is straightforward to implement.
