...
class EfficientAnnulusMesh(AbstractMesh):
    """
    Example usage
    ...
    def __init__(self, resolution=.0125,
                 path="/home/leonardo_mutti/PycharmProjects/masters_thesis/meshes/efficient_annulus/",
                 center=np.array([0, 0]), inner_radius=1, outer_radius=2, xdmf_path=None, ext_refinement=1.0,
                 int_refinement=1.0, power=8):
    ...
    def create_mesh(self):
        ...
        rb = self.resolution * self.ext_refinement
        rm = self.resolution * self.int_refinement

        # My surface
        plane_surface = model.add_plane_surface(circle.curve_loop, [hole.curve_loop])

        model.set_mesh_size_callback(
            # lambda dim, tag, x, y, z, lc: 4 * rm * (2 - r(x, y, z)) * (r(x, y, z) - 1) + rb   # seems like it is too difficult
            # lambda dim, tag, x, y, z, lc: 0.05 + (0.1 - 0.05) / (2 - 1) * (r(x, y, z) - 1)
            lambda dim, tag, x, y, z, lc: - (rm - rb) * (self.inner_radius - (self.outer_radius - r(x, y, z)) / (
